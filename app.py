...
# --- 住所（ファイルパス）の設定 ---
# ※GitHubの同じ箱の中にあるので、ファイル名だけでOK！
FILE_PATH = "FX_Auto_Hacking_Database_v3.xlsx"

# --- エクセルファイルを読み込んで画面に表示 ---
if not os.path.exists(FILE_PATH):
    # ファイルがない場合は日本語でメッセージを出す
    st.warning(f"⚠️ エクセルファイル（{FILE_PATH}）が同じ箱の中に見つかりません。GitHubにアップロードされているか確認してください。")
else:
    try:
        # エクセルを読み込む
            ...
            for index, row in df.iterrows():
                ...
                with col:
                    ...
                    <div style="border: 2px solid #e6e6e6; border-radius: 10px; padding: 15px; margin-bottom: 15px; background-color: #f9f9f9; box-shadow: 2px 2px 5px rgba(0,0,0,0.1);">
                        <h3 style="margin-top: 0; color: #1f77b4; border-bottom: 2px solid #1f77b4; padding-bottom: 5px;">{row.get('通貨ペア', '不明')}</h3>
                        ...
                        <p style="font-size: 18px; margin: 10px 0;"><strong>重要度：</strong> <span style="color: #ffaa00; font-size: 1.2em;">{row.get('重要度', '')}</span></p>
                        <div style="background-color: #ffffff; padding: 10px; border-radius: 5px; border: 1px dashed #ccc;">
                            <p style="margin: 0; font-size: 14px; color: #555;"><strong>📝コメント</strong><br>{row.get('コメント', '')}</p>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    
    except Exception as e:
        st.error(f"データの読み込み中にエラーが発生しました。エクセルの形式を確認してください。詳細: {e}")
